...
//	emit showMessage( tr("%1 bağlanıyor").arg(host) );
//}

//void Core::YTFailed(int /*error_number*/, QString /*error_str*/) {
//	emit showMessage( tr("Youtube sayfası alınamadı") );
//}

//void Core::YTNoVideoUrl() {
//	emit showMessage( tr("Videonun URL'si bulunamadı") );
//}
//#endif
...
//    qDebug("Core::screenshot");
         ...
         (QFileInfo(pref->screenshot_directory).isDir()) )
    {
        ...
        proc->takeScreenshot(PlayerProcess::Single, false/*pref->subtitles_on_screenshots*/);//kobe0417屏幕截图 path=~/图片/milis_video_screenshots/
//        qDebug("Core::screenshot: taken screenshot");
    }
    else {
        qDebug("Core::screenshot: error: directory for screenshots not valid");
        emit showMessage( tr("Ekran görüntüsü alınmadı, klasör yapılandırılmadı") );
    }
}
...
void Core::setAMarker(int sec) {
	...
	mset.A_marker = sec;
	displayMessage( tr("\"A\" işaretleyici %1 olarak ayarlandı").arg(Helper::formatTime(sec)) );
...
void Core::setBMarker(int sec) {
	...
	mset.B_marker = sec;
	displayMessage( tr("\"B\" işaretleyici %1 olarak ayarlandı").arg(Helper::formatTime(sec)) );
...
void Core::clearABMarkers() {
	...
	if ((mset.A_marker != -1) || (mset.B_marker != -1)) {
		mset.A_marker = -1;
		mset.B_marker = -1;
		displayMessage( tr("A-B işaretçiler temizlendi") );
		if (proc->isRunning()) restartPlay();
	}
...
void Core::setBrightness(int value) {
	...
	if (value != mset.brightness) {
		...
		displayMessage( tr("Parlaklık: %1").arg(value) );
		emit videoEqualizerNeedsUpdate();
	}
}
...
void Core::setContrast(int value) {
	...
	if (value != mset.contrast) {
		...
		mset.contrast = value;
		displayMessage( tr("Zıtlık: %1").arg(value) );
		emit videoEqualizerNeedsUpdate();
	}
}
...
void Core::setGamma(int value) {
	...
	if (value != mset.gamma) {
		...
		proc->setGamma(value);
		mset.gamma= value;
		displayMessage( tr("Gama: %1").arg(value) );
		emit videoEqualizerNeedsUpdate();
	}
}
...
void Core::setHue(int value) {
	...
	if (value != mset.hue) {
		...
		proc->setHue(value);
		mset.hue = value;
		displayMessage( tr("Renk: %1").arg(value) );
		emit videoEqualizerNeedsUpdate();
	}
}
...
void Core::setSaturation(int value) {
	...
	if (value != mset.saturation) {
		...
		displayMessage( tr("Canlılık: %1").arg(value) );
		emit videoEqualizerNeedsUpdate();
	}
}
...
//	qDebug("Core::setSpeed: %f", value);
	...
	displayMessage( tr("Hız: %1").arg(value) );
}
...
//        qDebug() << "-------------88888---------------muted=" << pref->mute;
	...
	displayMessage( tr("Ses: %1").arg(current_volume) );
...
void Core::setSubDelay(int delay) {
	...
	displayMessage( tr("Altyazı Gecikmesi: %1 ms").arg(delay) );
}
...
void Core::setAudioDelay(int delay) {
	...
	displayMessage( tr("Ses Gecikmesi: %1 ms").arg(delay) );
}
...
//			}
//			displayMessage( tr("Yazı tipi ölçeği: %1").arg(mset.sub_scale_ass) );
//		}
////	} else {
////		// No ass
...
////			}
////			displayMessage( tr("Yazı tipi ölçeği: %1").arg(mset.sub_scale) );
////		}
////	}
//}
...
//	qDebug("Core::changeSubVisilibity: %d", visible);
	...
	if (pref->sub_visibility) 
        displayMessage( tr("Altyazı Açık") );
	else
        displayMessage( tr("Altyazı Kapalı") );//字幕关闭
}
...
//	}
	...
	displayMessage( tr("En-boy oranı: %1").arg(asp_name) );
}
...
void Core::nextWheelFunction() {
	...
	case Preferences::Seeking:
		m = tr("Fare tekerleği şimdi istiyor");
		break;
	case Preferences::Volume:
		m = tr("Fare tekerleği şimdi sesi değiştiriyor");
		break;
	case Preferences::Zoom:
		m = tr("Fare tekerleği şimdi uzaklığı değiştiriyor");
		break;
	case Preferences::ChangeSpeed:
		m = tr("Fare tekerleği şimdi hızı değiştiriyor");
		break;
	}
	displayMessage(m);
}
...
//	qDebug("Core::changeZoom: %f", p);
	...
	displayMessage( tr("Yakınlaştır: %1").arg(mset.zoom_factor) );
}
...
//    qDebug("Core::displayScreenshotName: %s", filename.toUtf8().constData());
	...
	QString text = tr("Ekran görüntüsü %1 olarak kaydedildi").arg(fi.fileName());
...
void Core::displayUpdatingFontCache() {
	...
	emit showMessage( tr("Yazı tipi önbelleği güncelleniyor. Bu biraz zaman alabilir...") );
}
...
void Core::displayPlaying() {
	...
	emit showMessage(tr("Başlatılıyor..."));
}
