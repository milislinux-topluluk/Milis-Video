...
BaseGui * MilisVideo::main_window = 0;

MilisVideo::MilisVideo(const QString &arch, QObject * parent )
	: QObject(parent) 
{
    arch_type = arch;
...
BaseGui * MilisVideo::gui() {
	if (main_window == 0) {
		...
		if (move_gui) {
            qDebug("MilisVideo::gui: moving main window to %d %d", gui_position.x(), gui_position.y());
			main_window->move(gui_position);
		}
        if (resize_gui) {
            qDebug("MilisVideo::gui: resizing main window to %dx%d", gui_size.width(), gui_size.height());
...
BaseGui * MilisVideo::createGUI(QString arch_type/*QString gui_name*/) {
    BaseGui * gui = 0;
...
void MilisVideo::deleteGUI() {
    delete main_window;
    main_window = 0;
}

void MilisVideo::changeGUI() {//kobe 20170710
	deleteGUI();
...
MilisVideo::ExitCode MilisVideo::processArgs(QStringList args) {
	...
	for (int n=0; n < files_to_play.count(); n++) {
        qDebug("MilisVideo::processArgs: files_to_play[%d]: '%s'", n, files_to_play[n].toUtf8().data());
	}
    ...
    return MilisVideo::NoExit;
}

void MilisVideo::start() {
...
void MilisVideo::showInfo() {
    QString s = QObject::tr("This is Milis Vedio v. %1 running on %2")
