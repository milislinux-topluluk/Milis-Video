...
void SupportShortcuts::setData(Preferences * pref) {
    ...
    play_edit->append(tr("Oynat/Bekle:   Space"));
//    play_edit->append(tr("Önceki:     <, Media Previous"));
//    play_edit->append(tr("Sonraki:     >, Media Next"));
    play_edit->append(tr("Önceki:     %1").arg(pref->prev_key));
    play_edit->append(tr("Sonraki:     %1").arg(pref->next_key));

    QString line = QString(tr("İleri %1:     Right(→)").arg(Helper::timeForJumps(pref->seeking1)));
    play_edit->append(line);
    line = QString(tr("İleri %1:     Up(↑)").arg(Helper::timeForJumps(pref->seeking2)));
    play_edit->append(line);
    line = QString(tr("İleri %1:     PgUp").arg(Helper::timeForJumps(pref->seeking3)));
    play_edit->append(line);
    line = QString(tr("İleri %1:     Left(←)").arg(Helper::timeForJumps(pref->seeking1)));
    play_edit->append(line);
    line = QString(tr("İleri %1:     Down(↓)").arg(Helper::timeForJumps(pref->seeking2)));
    play_edit->append(line);
    line = QString(tr("İleri %1:     PgDn").arg(Helper::timeForJumps(pref->seeking3)));
    play_edit->append(line);
    play_edit->append(tr("Atla...:     Ctrl + J"));
    play_edit->append(tr("Sessiz:     M"));
    play_edit->append(tr("Ses +:     9"));
    play_edit->append(tr("Ses -:     0"));
    play_edit->append(tr("Ses gecikmesini ayarla:     Y"));
    play_edit->append(tr("Ses gecikmesini artırma veya azaltma:     + / - / ="));


    other_edit->append(tr("Oynatma Listesi:   %1").arg(pref->playlist_key));//F3
    other_edit->append(tr("Dosya Aç:     Ctrl + F"));
    other_edit->append(tr("Ekran Resmi:     S"));
    other_edit->append(tr("Özellikler:     Ctrl + P"));
    other_edit->append(tr("Bilgileri ve özellikleri görüntüle...:     Ctrl + I"));
    other_edit->append(tr("Hakkında:   Ctrl + A"));
    other_edit->append(tr("Çıkış:   Ctrl + Q"));
    other_edit->append(tr("Tam Ekran Aç/Kapat:   Ctrl + Enter"));
