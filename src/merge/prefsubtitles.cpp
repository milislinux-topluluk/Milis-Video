...
void PrefSubtitles::createHelp() {
	...
	addSectionTitle(tr("Altyazılar"));

	setWhatsThis(font_autoload_combo, tr("Oto. Yükle"), 
        tr("Altyazı otomatik yükleme yöntemini seçin.") );

	setWhatsThis(font_encoding_combo, tr("Varsayılan altyazı kodlaması"), 
        tr("Select the encoding which will be used for subtitle files "
           "by default.") );

	setWhatsThis(use_enca_check, tr("Bu dil için otomatik algılamayı deneyin"),
	...
	setWhatsThis(enca_lang_combo, tr("Altyazı dili"),
