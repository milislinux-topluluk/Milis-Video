...
{
	setupUi(this);
    hwdec_combo->addItem(tr("Yok"), "no");
	hwdec_combo->addItem(tr("Oto"), "auto");
...
void PrefPerformance::createHelp() {
	...
	setWhatsThis(hwdec_combo, tr("Hardware decoding"),
		...
		tr("Available options:") +
			...
			"</ul>" +
		tr("Bu seçenek yalnızca mpv ile çalışır."));

	addSectionTitle(tr("Önbellek"));

	setWhatsThis(cache_files_spin, tr("Dosyalar için önbellek"), 
		tr("This option specifies how much memory (in kBytes) to use when "
           "precaching a file.") );

	setWhatsThis(cache_streams_spin, tr("Akışlar için önbellek"), 
		tr("This option specifies how much memory (in kBytes) to use when "
           "precaching a URL.") );
}
