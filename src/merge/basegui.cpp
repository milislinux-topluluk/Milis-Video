...
#include "../milis/titlewidget.h"
#include "../milis/bottomwidget.h"
#include "../milis/playmask.h"
#include "../milis/aboutdialog.h"
#include "../milis/esctip.h"
#include "../milis/tipwidget.h"
#include "audiodelaydialog.h"
#include "../milis/messagedialog.h"
...
//#include "../milis/shortcutswidget.h"
#include "../milis/helpdialog.h"
#include "inputurl.h"
...
//    shortcuts_widget->set_parent_widget(this);
    ...
    tipWidget = new TipWidget("Hello, Milis!", this);
...
void BaseGui::createActionsAndMenus() {
    ...
    clearRecentsAct->change(QPixmap(":/res/delete_normal.png"), tr("&Temizle"));

    recentfiles_menu = new QMenu(this);
    recentfiles_menu->addSeparator();
    recentfiles_menu->addAction(clearRecentsAct);
    recentfiles_menu->menuAction()->setText(tr("Geçmiş Dosyalar"));
...
//    speed_menu->menuAction()->setIcon(QPixmap(":/res/speed.png"));
    ...
    normalSpeedAct->change( tr("Normal Hız"));
    halveSpeedAct->change(tr("Yarım Hız") );
    doubleSpeedAct->change(tr("İkikat Hız"));
    decSpeed10Act->change(tr("Hız -10%"));
    incSpeed10Act->change(tr("Hız +10%"));
    decSpeed4Act->change(tr("Hız -4%"));
    incSpeed4Act->change(tr("Hız +4%"));
    decSpeed1Act->change(tr("Hız -1%"));
    incSpeed1Act->change(tr("Hız +1%"));
    ...
    aspect_menu->menuAction()->setText(tr("En-boy Oranı"));//宽高比  画面比例
...
//    shortcutsAct->change(tr("Kısayollar"));
    ...
    ontop_menu->menuAction()->setText(tr("Her Zaman Üstte"));
//	ontop_menu->menuAction()->setIcon(Images::icon("ontop"));
    ...
    play_order_menu->menuAction()->setText(tr("Sıradakini Oynat"));
//    play_order_menu->menuAction()->setIcon(Images::icon("list_cycle_normal"));
    ...
    audiochannels_menu->menuAction()->setText(tr("&Kanallar"));

    audioMenu = new QMenu(this);
    audioMenu->menuAction()->setText(tr("Ses"));
//    audioMenuAct->setIcon(Images::icon("audio_menu"));
    ...
    muteAct->change(icset, tr("&Sessiz"));
    ...
    stereomode_menu->menuAction()->setText(tr("&Steryo Modu"));
//	stereomode_menu->menuAction()->setIcon(Images::icon("stereo_mode"));
    ...
    subtitlesMenu->menuAction()->setText( tr("Altyazılar") );
//    subtitlesMenuAct->setIcon(Images::icon("subtitles_menu"));
    ...
    helpAct->change(QPixmap(":/res/help_normal.png"), tr("Yardım"));

    aboutAct = new MyAction(QKeySequence("Ctrl+A"), this, "about_milis_video");
    connect(aboutAct, SIGNAL(triggered()), this, SLOT(showAboutDialog()));
    aboutAct->change(Images::icon("about_normal"), tr("&Milis Video Hakkında"));

    quitAct = new MyAction(QKeySequence("Ctrl+Q"), this, "quit");
    quitAct->change(Images::icon("quit_normal"), tr("Çıkış"));
...
//    this->activate_timer();
    ...
    if ((pref->vdpau.disable_video_filters) && (pref->vo.startsWith("vdpau"))) {
        ...
        mirrorAct->setEnabled(false);
        rotateGroup->setActionsEnabled(false);
        displayMessage(tr("Vdpau kullanılırken video filtreleri devre dışı bırakıldı"));
    }
}
...
//            qDebug() << "kobe recents=" << QString("%1. " + show_name ).arg( i.insert( i.size()-1, '&' ), 3, ' ' );
        ...
        QAction * a = recentfiles_menu->addAction( tr("<boş>") );
        a->setEnabled(false);
    }
...
void BaseGui::clearRecentsList() {
    MessageDialog msgDialog(0, tr("Silmeyi Onayla - Milis Video"), tr("Son dosyalar listesi silinsin mi?"));
    if (msgDialog.exec() != -1) {
...
void BaseGui::openURL() {
    ...
    QString s = QInputDialog::getText(this,
            tr("Milis Video - URL Gir"), tr("URL:"), QLineEdit::Normal,
            pref->last_url, &ok );
...
void BaseGui::gotForbidden() {
    ...
    QMessageBox::warning(this, tr("Hata tespit edildi"),
        tr("Maalesef bu video oynatılamıyor.") +"<br>"+
        tr("Sunucu '%1' i döndürdü").arg("403: Forbidden"));
	busy = false;
}
...
void BaseGui::displayGotoTime(int t) {
    ...
    QString s = tr("%1 atla").arg( Helper::formatTime(jump_time) );
    if (pref->fullscreen) {
...
{
    ErrorDialog d;
    d.setWindowTitle(tr("%1 Hatası").arg(PLAYER_NAME));
    d.setTitleText(tr("%1 Hatası").arg(PLAYER_NAME));
    d.setText(tr("'%1' bulunamadı!").arg(errorStr));
    d.hideDetailBtn();
    d.exec();
}
...
void BaseGui::showExitCodeFromMplayer(int exit_code) {
	...
	if (exit_code != 255 ) {
		...
		d.setWindowTitle(tr("%1 Hatası").arg(PLAYER_NAME));
        d.setTitleText(tr("%1 Hatası").arg(PLAYER_NAME));
		d.setText(tr("%1 beklenmedik şekilde bitti.").arg(PLAYER_NAME) + " " + 
	              tr("Çıkış Kodu: %1").arg(exit_code));
...
//	qDebug("BaseGui::showErrorFromMplayer");
	...
	if ((e == QProcess::FailedToStart) || (e == QProcess::Crashed)) {
		...
		if (e == QProcess::FailedToStart) {
			d.setText(tr("%1 başlatılamadı.").arg(PLAYER_NAME) + " " + 
                         tr("Lütfen tercihlerdeki %1 yolunu kontrol edin.").arg(PLAYER_NAME));
		} else {
			d.setText(tr("%1 çöktü.").arg(PLAYER_NAME) + " " + 
                      tr("Daha fazla bilgi için günlüğe bakın."));
		}
        d.setLog(mplayer_log);
		d.exec();
	}
...
//            d.setText(tr("%1 okunamadı.").arg(PLAYER_NAME) + " " +
//                         tr("Lütfen %1 dosyasının var olup olmadığını kontrol edin."));
//        } else {
//            d.setText(tr("%1 yazılamadı.").arg(PLAYER_NAME) + " " +
//                      tr("Lütfen dosyanın hala olup olmadığını kontrol edin."));
//        }
//        d.setLog(mplayer_log);
//        d.exec();
//    }
