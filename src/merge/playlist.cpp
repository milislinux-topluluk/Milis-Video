...
#include "../milis/playlistitem.h"
#include "../milis/playlistview.h"
...
void Playlist::createNoVideo()
{
    ...
    novideo_text->setText(tr("Çalma listesi boş"));
...
//    titleLabel->setFixedSize(220-156, 36);
    ...
    removeSelectedAct->change(tr("Seçimi kaldır"));
    ...
    deleteSelectedFileFromDiskAct->change(tr("&Dosyayı diskten sil") );
...
//            emit this->finish_list();
            emit this->showMessage(tr("Oynatma listesinin sonuna ulaştı"));
        }
        else {
            playItem(current_item + 1);
        }
...
void Playlist::popupDialogtoSelectFiles() {
    ...
    QStringList files = MyFileDialog::getOpenFileNames(
                            this, tr("Açılacak bir veya daha fazla dosya seçin"),
                            lastDir(),
                            tr("Multimedya") + e.multimedia().forFilter() + ";;" +
                            tr("Tüm Dosyalar") +" (*.*)" );
...
void Playlist::removeTheSelected()
{
    ...
    if (!filename.isEmpty()) {
        MessageDialog msgDialog(0, tr("Çıkarmayı Onayla"),
                                tr("'%1' dosyasını çalma listesinden kaldırmak üzeresiniz.").arg(filename) + "<br>"+
                                tr("Devam etmek istediğinden emin misin?"), QMessageBox::Yes | QMessageBox::No);
        if (msgDialog.exec() != -1) {
...
{
    if (!filename.isEmpty()) {
        MessageDialog msgDialog(0, tr("Kaldırmayı Onayla"),
                                tr("'%1' dosyasını çalma listesinden kaldırmak üzeresiniz.").arg(filename) + "<br>"+
                                tr("Devam etmek istediğinden emin misin?"), QMessageBox::Yes | QMessageBox::No);
        if (msgDialog.exec() != -1) {
            if (msgDialog.standardButton(msgDialog.clickedButton()) == QMessageBox::Yes) {
...
//		listView->setCurrentCell(current, 0);
        ...
        if (fi.exists() && fi.isFile() && fi.isWritable()) {
            ...
            MessageDialog msgDialog(0, tr("Silmeyi Onayla"),
                                    tr("Sürücünüzden '%1' dosyasını SİLMEK üzeresiniz.").arg(filename) + "<br>"+
                                    tr("Bu eylem geri alınamaz. Devam etmek istediğinden emin misin?"), QMessageBox::Yes | QMessageBox::No);
            if (msgDialog.exec() != -1) {
                if (msgDialog.standardButton(msgDialog.clickedButton()) == QMessageBox::Yes) {
                    // Delete file
...
//                        QMessageBox::warning(this, tr("Silme Hatası"),
//                            tr("'%1' silinemedi").arg(filename));
                        MessageDialog warnDialog(0, tr("Silme Hatası"), tr("'%1' silinemedi").arg(filename), QMessageBox::Ok);
                        warnDialog.exec();
                    }
                }
            }
...
//            qDebug("Playlist::deleteSelectedFileFromDisk: file doesn't exists, it's not a file or it's not writable");
            MessageDialog infoDialog(0, tr("Dosya silme hatası"), tr("Dosya sisteminden '%1' dosyasını silmek mümkün değildir.").arg(filename), QMessageBox::Ok);
            infoDialog.exec();
        }
    }
}
