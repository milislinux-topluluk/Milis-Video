...
void PrefVideo::updateDriverCombos() {
	...
	vo_combo->clear();
	vo_combo->addItem(tr("Varsayılan"), "player_default");
	QString vo;
...
//			vo_combo->addItem( "gl (" + tr("hızlı") + ")", "gl:yuv=2:force-pbo");//kobe
//			vo_combo->addItem( "gl (" + tr("hızlı - ATI kartlar") + ")", "gl:yuv=2:force-pbo:ati-hack");//kobe
...
//	vo_combo->addItem( tr("Kullanıcı tanımlı..."), "user_defined" );//kobe:去掉自定义驱动选项
...
void PrefVideo::createHelp() {
	...
	setWhatsThis(vo_combo, tr("Video çıkış sürücüsü"),
		tr("Video çıkışı sürücüsünü seçin. %1 en iyi performansı sağlar.")
		  .arg("<b><i>xv</i></b>")
		);

	setWhatsThis(postprocessing_check, tr("Sonra işleme özelliğini varsayılan olarak etkinleştir"),
		tr("Post işleme, yeni açılan dosyalarda varsayılan olarak kullanılacaktır.") );

	setWhatsThis(eq2_check, tr("Yazılım video ekolayzer"),
	...
	setWhatsThis(direct_rendering_check, tr("Doğrudan işleme"),
	...
	setWhatsThis(double_buffer_check, tr("Çift tamponlama"),
	...
	setWhatsThis(use_slices_check, tr("Dilimleri kullanarak video çiz"),
